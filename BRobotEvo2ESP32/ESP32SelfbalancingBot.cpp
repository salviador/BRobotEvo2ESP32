...
void initWifiAP() {
	...
	if (WiFi.softAP("Robot-Nicola", "123456710")) {
...
//	Wire.begin();
  Wire.begin(21,22,400000); // Pins 21 und 22 bei 400kHz 
/*
pinMode(SDA,INPUT_PULLUP);
pinMode(SCL,INPUT_PULLUP);
Wire.begin(SDA, SCL); // 21 & 22 are default on ESP32
...
*/
  ...
  Serial.println("Init TImer Finito");

	OSC_init();

  Serial.println("Osc Finito");
...
void loop() {
	...
	if (MPU6050_newData()) {
		...
		if (OSCpush[0])     // If we press the SERVO button we start to move
			angle_ready = 92;  //82 valore originale
		else
			angle_ready = 68;  // 74 valore originale  Default angle
	...
	if (loop_counter >= 15) {
		...
		char auxS1[50];
		sprintf(auxS1, "$tD,%d,%d,%ld", int(angle_adjusted * 10), int(estimated_speed_filtered), steps1);
		OSC_MsgSend(auxS1, 50);
#endif
