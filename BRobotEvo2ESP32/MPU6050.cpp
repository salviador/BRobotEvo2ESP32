...
int MPU6050_read(int start, uint8_t *buffer, uint8_t size1)
{
  int i, n, error;
  ...
  Wire.requestFrom((int) MPU6050_I2C_ADDRESS, (uint8_t) size1, (bool) true);
  i = 0;
  while (Wire.available() && i < size1)
  {
    buffer[i++] = Wire.read();
  }
  if ( i != size1)
...
int MPU6050_write(int start, const uint8_t *pData, int size1)
{
  int n, error;
  ...
  n = Wire.write(pData, size1);  // write data bytes
  if (n != size1)
    return (-21);
